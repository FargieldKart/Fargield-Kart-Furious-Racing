...
Move the modded 'localization' file to the same Windows folder (Note: If you are updating from an earlier version of Fargield Kart, replace the old modded localization file with the one from this version)
...
Follow the same instructions as above, but download BepInEx 5.4.21 x64, go to the GKFR page in your Steam library, click on Manage (cog icon) then Properties, and add this command in Launch Options: WINEDLLOVERRIDES="winhttp.dll=n,b" %command%
...
5. "That looks nothing like Fargield. It's a recolored Garfield." *sighs* I know. I wish I knew how to remodel 3D assets, so he actually looks like the real deal.
